...
def test_preserves_round_key_count(round_count: int) -> None:
    ...
    decryption_round_keys, _ = build_decryption_round_keys(
        encryption_round_keys=encryption_round_keys[:-1],
        final_encryption_key=encryption_round_keys[-1],
    )
    assert len(decryption_round_keys) == round_count - 1
...
def test_last_decryption_key_is_first_encryption_key() -> None:
    ...
    _, final_decryption_key = build_decryption_round_keys(
        encryption_round_keys=encryption_round_keys[:-1],
        final_encryption_key=encryption_round_keys[-1],
    )
    assert final_decryption_key == encryption_round_keys[0]
...
def test_accepts_aes128_expansion(key: bytes) -> None:
    ...
    decryption_round_keys, final_decryption_key = build_decryption_round_keys(
        encryption_round_keys=encryption_round_keys,
        final_encryption_key=final_encryption_key,
    )
    assert len(decryption_round_keys) == AES_128_ROUND_COUNT
...
def test_accepts_aes192_expansion(key: bytes) -> None:
    ...
    assert len(decryption_round_keys) == AES_192_ROUND_COUNT
    assert decryption_round_keys[0] == final_encryption_key
    assert final_decryption_key == encryption_round_keys[0]
...
def test_accepts_aes256_expansion(key: bytes) -> None:
    ...
    assert len(decryption_round_keys) == AES_256_ROUND_COUNT
    assert decryption_round_keys[0] == final_encryption_key
    assert final_decryption_key == encryption_round_keys[0]
